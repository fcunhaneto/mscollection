...
import texts
from db.db_model import Category, MovieCategory, SeriesCategory
from db.db_settings import Database as DB
from lib.function_lib import hbox_create, pb_create, delete_orphans


class DeleteOrphansCategory(QMdiSubWindow):
    def __init__(self, main):
        """
        Class for delete categories who are orphan in database.

        :param main: Reference for main windows.
        """
        super(DeleteOrphansCategory, self).__init__()

        self.session = DB.get_session()
        self.category_id = self.session.query(Category.id)
        self.categories = self.session.query(Category)
        self.main = main

        window_title = texts.delete_orphans + ' ' + texts.category_p
        ...
        self.setMaximumHeight(int(0.8 * main.frameSize().height()))
    ...
    def create_table(self):
        """
        Create a table for show all orphan categories info and with a QCheckBox
        that if is check the actor will be delete.
        """
        category_movies = self.category_id.\
            filter(MovieCategory.category_id == Category.id)
        sub_category_movies = category_movies.distinct()

        category_series = self.category_id.\
            filter(SeriesCategory.category_id == Category.id)
        sub_category_series = category_series.distinct()

        category_result = self.categories.\
            filter(and_(Category.id.notin_(sub_category_movies),
                        Category.id.notin_(sub_category_series)))\
            .all()
        ...
        for category in category_result:
            ...
            ch_del = QCheckBox(str(category.id))
    ...
    def delete(self):
        ...
        """
        delete_orphans(self.session, self.ch_del, Category, texts.category_s)
        self.clear()
        self.create_table()
