...
import texts
from db.db_model import Actor, Cast
from db.db_settings import Database as DB
from lib.function_lib import hbox_create, pb_create, delete_orphans


class DeleteOrphansActor(QMdiSubWindow):
    def __init__(self, main):
        """
        Class for delete actors who are orphan in database.

        :param main: Reference for main windows.
        """
        super(DeleteOrphansActor, self).__init__()

        self.session = DB.get_session()
        self.actors = self.session.query(Actor)
        self.main = main

        window_title = texts.delete_orphans + ' ' + texts.actor_p
    ...
    def create_table(self):
        """
        Create a table for show all orphan actors info and with a QCheckBox
        that if is check the actor will be delete.
        """
        sub = self.session.query(Cast.actor_id)
        sub = sub.distinct()
        actor_result = self.actors.filter(Actor.id.notin_(sub)).all()

        for actor in actor_result:
            self.table.insertRow(self.rows)
            self.table.setItem(self.rows, 0, QTableWidgetItem(actor.name))

            ch_del = QCheckBox(str(actor.id))
    ...
    def delete(self):
        ...
        """
        delete_orphans(self.session, self.ch_del, Actor, texts.actor_s)
        self.clear()
        self.create_table()
