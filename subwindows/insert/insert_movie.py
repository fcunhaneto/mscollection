...
from db.db_model import Director, Box, Keyword, Movie, MovieCategory, \
...
class InsertMovie(QMdiSubWindow):
    ...
    def __init__(self, main):
        ...
        self.pb_search_imdb = pb_create(texts.pb_imdb_ad_search, height=25,
                                        width=200)
        self.pb_search_imdb.clicked.\
            connect(lambda type: self.set_movie_url('imdb'))
        ...
        # TODO corrigir a tecla de atalho CTRL+Shift+A
        ...
        self.pb_save.clicked.connect(self.start_save_movie)
    ...
    def insert_movie(self):
        ...
        for i in range(2):
            ...
            if category:
                movie_category = MovieCategory(order=i + 1, category=category)
                self.movie.categories.append(movie_category)
        ...
        self.movie.last_edit = hour_now
        self.movie.new_edit = hour_now
        result = db_insert_obj(self.session, self.movie)

        if result:
            self.movie.view_url = write_html(self.session, self.movie, 'movie')
    ...
    # Save Movie
    def start_save_movie(self):
        """
        Calls the insert movie method and produces messages for possible error.
        """
    ...
    def refresh_combobox(self):
        ...
        populate_combobox(self.cb_director,
                          db_select_all(self.session, Director))
        self.clear_table()
