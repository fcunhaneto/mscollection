...
from PyQt5.QtWidgets import QMdiSubWindow, QWidget, QFormLayout, QVBoxLayout, \
    ...
    QMessageBox, QSpacerItem, QSizePolicy, QTableWidget
...
from db.db_model import Creator, Keyword, Series, SeriesCategory, \
    SeriesCreator, SeriesCast
...
from lib.adoro_cinema_scraping_series import AdoroCinemaSeriesScraping
from lib.write_ms_html import write_html


class InsertSeries(QMdiSubWindow):
    """
    Class to provide all methods to insert series in database
    """
    def __init__(self, main):
        super(InsertSeries, self).__init__()
        ...
        self.pb_search_imdb = pb_create(texts.pb_imdb_search_confirm, height=25,
                                        width=200)
        self.pb_search_imdb.clicked. \
            connect(lambda type: self.set_series_url('imdb'))
        self.pb_search_imdb.setShortcut('CTRL+Shift+I')
        self.pb_search_imdb.setEnabled(False)
        self.pb_search_imdb.setHidden(True)

        self.pb_search_ad = pb_create(texts.pb_ad_search_confirm, height=25,
                                      width=300)
        self.pb_search_ad.clicked.connect(lambda type: self.set_series_url('ad'))
        self.pb_search_ad.setShortcut('CTRL+Shift+D')
        self.pb_search_ad.setEnabled(False)
        self.pb_search_ad.setHidden(True)

        self.hbox_search = QHBoxLayout(self.subwindow)
        self.hbox_search.addWidget(self.pb_search_confirm_imdb)
        self.hbox_search.addWidget(self.lb_imdb_url)
        self.hbox_search.addWidget(self.le_imdb_url)
        self.hbox_search.addWidget(self.pb_search_confirm_ad)
        self.hbox_search.addWidget(self.lb_ad_url)
        self.hbox_search.addWidget(self.le_ad_url)

        self.hbox_search.setContentsMargins(20, 10, 20, 0)

        self.hbox_pb_search = QHBoxLayout(self.subwindow)
        ...
        self.pb_save.clicked.connect(self.save_series)
    ...
    def insert_series(self):
        ...
        if self.le_poster.text():
            ...
            path = os.getcwd()
            self.series.poster = path + '/images/poster_placeholder.png'

        self.series.url = self.le_url.text()
        self.series.summary = self.le_summary.toPlainText()

        id, name = get_combobox_info(self.cb_media)
        self.series.media_id = db_get_id(
            self.session, id, name, Media(name=name)
        )
        ...
        for i in range(2):
            ...
            if category:
                series_category = SeriesCategory(order=i + 1, category=category)
                self.series.categories.append(series_category)

        id, name = get_combobox_info(self.cb_creator)
        creator = db_get_obj(self.session, id, name, Creator)
        if creator:
            series_creator = SeriesCreator(order=1, creator=creator)
            self.series.creators.append(series_creator)
        ...
        for cb in self.cb_actor:
            ...
            if actor_id and character_id:
                ...
                if cast:
                    ...
                    self.series.series_cast.append(series_cast)

            i += 1

        hour_now = datetime.datetime.utcnow()
        self.series.date_create = hour_now
        self.series.last_edit = hour_now
        self.series.new_edit = hour_now

        result = db_insert_obj(self.session, self.series)

        if result:
            self.series.view_url = write_html(self.session, self.series,
                                             'series')
            self.session.add(self.series)
            self.session.commit()
            return self.errors['no error']
        else:
            return self.errors['db error']

    # Save Series
    def save_series(self):
        """
        Calls the insert series method and produces messages for possible error.
        :return:
        """
        result = self.insert_series()
        if result == self.errors['no error']:
            ...
            self.pb_search_imdb.setHidden(True)
            self.clear()
        if result == self.errors['no title']:
            show_msg(
                texts.insert_error, texts.no_title,
                QMessageBox.Warning,
                QMessageBox.Close
            )
        if result == self.errors['db error']:
    ...
    def clear_table(self):
        """
        Clear a table after saving series in database or clear button is
        clicked.
        :return:
        """
        self.table.clear()
    ...
    def clear(self):
        """
        Clear all input values after saving series in database or clear button is
        clicked.
        :return:
        """
    ...
    def search_confirmed_imdb(self):
        ...
        self.le_imdb_url.returnPressed. \
            connect(lambda type='imdb': self.set_series_url(type))
        self.le_ad_url.setEnabled(False)
        self.le_ad_url.setHidden(True)
    ...
    def search_confirmed_ad(self):
        ...
        self.le_ad_url.returnPressed. \
            connect(lambda type='ad': self.set_series_url(type))
        self.pb_search_imdb.setEnabled(False)
        self.pb_search_imdb.setHidden(True)
    ...
    # Imdb Serch
    def set_series_url(self, type):
        """
        Running class Imdb Search on file lib/imdb_scraping.py to get values from
        IMDB url and than populate fields with this values.
        :return:
        """
        result = None
        if type == 'imdb':
            imdb_search = ImdbScraping(self.le_imdb_url.text(), 'series')
            result = imdb_search.get_values()
        elif type == 'ad':
            ad_search = AdoroCinemaSeriesScraping(self.le_ad_url.text())
        ...
        if result['director_creator']:
            self.set_combobox_value(self.cb_creator, Creator,
                                    result['director_creator'])
        ...
        if result['categories']:
            ...
            for i in range(total):
                ...
                self.set_combobox_value(self.cb_categories[i], Category, name)
    ...
    def set_combobox_value(self, cb, obj, name):
        """
        Look in a combobox for an existing value to appear in the combobox's
        first line.
        :param cb: combobox
        :param obj: The class representing the database table that populates
        this combobox
        :param name: The name to search in combobox
        :return:
        """
        try:
